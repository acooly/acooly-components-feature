...
function change(k, v) {
    var str = "0";
    for (i = 0; i < v; i++) {
        str += ("_" + (document.getElementById(ids[k][i]).selectedIndex - 1));
    }
    ;

    var ss = document.getElementById(ids[k][v]);
    with (ss) {
        length = 0;
        options[0] = new Option(opt0[k][v], opt0[k][v]);
        if (v && document.getElementById(ids[k][v - 1]).selectedIndex > 0 || !v) {
            if (dsy.Exists(str)) {
                ar = dsy.Items[str];
                for (j = 0; j < ar.length; j++) {
                    options[length] = new Option(ar[j], ar[j]);
                }//end for
                if (v) {
...
function _init_area() {  //初始化函数
    for (var i = 0; i < ids.length; i++) {
        for (var j = 0; j < ids[i].length - 1; j++) {
            document.getElementById(ids[i][j]).onchange = new Function("change(" + (i) + "," + (j + 1) + ")");
        }
        change(i, 0);
    }
}
...
function printJson() {
    var json = "{";
    var provs = dsy.Items["0"];
    for (i = 0; i < provs.length; i++) {
        if (i > 0) json += ",";
        json += "\"" + provs[i] + "\":";
        var ar = dsy.Items["0_" + i];
        var citys = "["
        for (j = 0; j < ar.length; j++) {
            if (j > 0) {
                citys += ","
